...
async function main() {
  ...
  let failTripOnce = false;
  let controlledTurn = false;
  let releaseTurn;
  ...
  await context.route("**/*", async (route) => {
    ...
    if (
      failTripOnce &&
      url.pathname.endsWith("/trip") &&
      request.method() === "PUT"
    ) {
      failTripOnce = false;
      expectedFailures.push(request.url());
      await route.fulfill({
        status: 503,
        json: {
          error: {
            code: "ui7_controlled_failure",
            message: "离线测试：保存暂不可用，请重试。",
          },
        },
      });
      return;
    }
    if (controlledTurn && url.pathname.endsWith("/advisor-turns")) {
      expectedFailures.push(request.url());
      await new Promise((resolve) => {
        releaseTurn = resolve;
      });
      controlledTurn = false;
      await route.fulfill({
        status: 503,
        json: {
          error: {
            code: "ui7_controlled_failure",
            message: "离线测试：发送暂不可用，草稿已保留。",
          },
        },
      });
      return;
    ...
    if (
      ...
      if (body.message === "UI7 顾问恢复检查")
        advisor = {
          ...advisor,
          phase: "degraded",
          pending_suggestions: [],
          safety_summary: "离线示例：顾问暂不可用，仍可使用地点清单。",
        };
      else {
  ...
  const assertConsole = () => {
    const remaining = [...expectedFailures];
    const unexpectedErrors = errors.filter((error) => {
      const index = remaining.indexOf(error.url);
      if (
        index >= 0 &&
        error.type === "error" &&
  ...
  try {
    ...
    const draft = page.getByRole("textbox", { name: "你的补充" });
    controlledTurn = true;
    await draft.fill("UI7 顾问恢复检查");
    await page.getByRole("button", { name: "发送给旅行顾问" }).click();
    await page
      .getByRole("button", { name: "发送给旅行顾问" })
      .isDisabled()
      .then((value) => assert.equal(value, true));
    await capture("advisor-sending", ".f019-composer");
    while (!releaseTurn)
      await new Promise((resolve) => setTimeout(resolve, 10));
    releaseTurn();
    await page
      .getByText("本地请求暂时无法完成，请稍后重试。", { exact: true })
      .waitFor();
    assert.equal(await draft.inputValue(), "UI7 顾问恢复检查");
    await capture("advisor-send-failed", ".f019-composer");
    await page.getByRole("button", { name: "发送给旅行顾问" }).click();
    await page
      .getByText("顾问暂不可用。离线示例：顾问暂不可用，仍可使用地点清单。", {
        exact: true,
      })
      .waitFor();
    await capture("advisor-degraded", ".f019-advisor");
    await page
    ...
    const budget = await page
      .getByRole("textbox", { name: "总预算（CNY）", exact: true })
      .inputValue();
    await page
      .getByRole("button", { name: "保存信息并进入空间预检", exact: true })
      .click();
    await page
      .getByText("旅行信息暂时无法保存，请稍后重试。", { exact: false })
      .first()
      .waitFor();
    assert.equal(
      await page
    ...
    await capture("details-save-failed", ".f012-save-feedback");
    await page
      .getByRole("button", { name: "保存信息并进入空间预检", exact: true })
      .click();
    await page
    ...
    await capture(
      "result-timeline",
      '.f011-timeline[aria-label="第 1 日行程"]',
    );
    const dayButtons = page.locator(".f009-map-days button");
    if ((await dayButtons.count()) > 1) {
      await dayButtons.nth(1).click();
      await capture("result-day-2", ".f009-map-shell");
    }
    await page.getByText("查看技术详情", { exact: true }).click();
    await capture("result-diagnostic", ".f010-model-diagnostic");
    await page
    ...
    report.expectedFailures = expectedFailures;
    report.actions.push(
      "send in-flight disabled",
      "failed draft retained then degraded advisor",
      "failed save retained budget and retried",
      "result day controls and diagnostic disclosure",
    );
    report.status = "PASS";
  } catch (error) {
    report.status = "FAIL";
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        { requests, unexpected, errors, actions, expectedFailures },
        null,
        2,
      ),
    );
