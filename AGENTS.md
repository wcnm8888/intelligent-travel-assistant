...
- 已接入 2026-09-07 方法论改进，入口见 [测试策略：方法论接入与失败处理](./docs/testing-strategy.md#方法论接入与失败处理)。失败、warning、QA 就绪和证据以中央专项规范为准，项目任务卡维护实际授权及例外。
- 普通本地开发可以按已批准范围和额度完成诊断—修复—定向复验，不逐行申请；只读任务、冻结正式验收、真实 Provider UAT 和已经触发的专项停止不能据此自动恢复。
...
出现以下任一情况时停止相关动作并请求用户确认：需要扩大任务范围、使用真实凭证或外部服务、进行远程写入、修改系统环境、删除或覆盖材料、触及目标项目外文件、未知状态漂移，或达到任务修复额度/同一问题连续三次修复失败。失败轮次与预期 RED 的区分按中央质量门禁执行，不把首次本地检查失败一律解释为禁止范围内修复；更严格的专项停止条件优先，已触发的暂停须明确恢复。
