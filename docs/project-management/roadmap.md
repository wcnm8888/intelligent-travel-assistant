...
目标：在已完成的 B-000 工程底座上交付首个真实、狭窄、端到端的旅行规划任务。F-001 任务卡已获批准，当前处于实施期。

| 顺序 | 任务 | 状态 | 用户价值 | 关键依赖 |
| --- | --- | --- | --- | --- |
| 0 | B-000 项目与工程基线 | DONE | 已建立可开发、可运行、可验证、可交接的本地工程底座；PR #1 已合并 | 无 |
| 1 | F-001 单城市双日旅行计划垂直切片 | ACTIVE | 用户能用真实天气、POI 和路线生成第一份可校验计划 | B-000、外部服务就绪门禁 |
| 2 | F-002 计划持久化、来源与版本 | CANDIDATE | 用户能保存、恢复和追踪计划版本及来源 | F-001 |
...
- 当前交付状态：Step 45D 已在不放宽最终校验的前提下修复候选路线正数时间窗口准入；最后一次受控 Step 45E 证明新准入在 live 路径生效，但 generation 与唯一一次 repair 均未生成时间可行候选，真实计划 UAT 仍为 `FAIL`。Step 45F 已将时间失败细分为五类安全诊断并确认原有 Prompt、上下文和冻结规则完整；D-009 与 F-001-CR1 九项实施决策已经批准，Step 45G 已完成无最终时间 proposal、实际路线、确定性 scheduler 和独立复验的详细设计，但生产代码尚未迁移。Step 45H 仍需单独批准，完成前不得进入 Step 46；
- 范围处置：用户已批准 F-001 作为一个完整垂直切片由单一 PR 交付，并接受截至 Step 41 的 120 个变更文件任务级范围例外；该例外不扩大产品范围，也不降低 review、CI 或安全门禁。Step 45H 采用 stacked PR，并获批新增 24–34 个文件影响和 1200–2200 行范围例外；超过上界必须重新审批。
...
- 状态：`CANDIDATE`
- 目标：从双日单城市扩展到更长行程和跨城市交通约束；
- 核心价值：覆盖中国大陆境内更真实的自由行路线；
- 必须验证：住宿城市连续性、跨城交通、相邻日期依赖、天气可用范围、全费用类别和未知费用；
- 数据限制：没有权威实时票价或库存时使用 `unknown`、用户输入或明确估算；
- 非目标：出票、预订、实时库存承诺和境外行程。
...
上述账户、凭证、条款确认和首次 live 授权已在 Step 37–38 满足，并取得一次三家 provider 的脱敏 live 契约证据。Step 39 的通过式真实计划 UAT 仍未满足；这不撤销账户就绪事实，也不授权 Codex 登录、付费或再次调用真实 API。任何新的 live 回归必须重新确认调用次数、费用和停止条件。
