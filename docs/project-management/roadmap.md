...
目标：先完成 B-000 工程底座，再交付一个真实、狭窄、端到端的旅行规划垂直切片。当前不并行启动业务功能。

| 顺序 | 任务 | 状态 | 用户价值 | 关键依赖 |
| --- | --- | --- | --- | --- |
| 0 | B-000 项目与工程基线 | ACTIVE | 建立可开发、可运行、可验证、可交接的本地工程底座 | 无 |
| 1 | F-001 单城市双日旅行计划垂直切片 | CANDIDATE | 用户能用真实天气、POI 和路线生成第一份可校验计划 | B-000、外部服务就绪门禁 |
...
B-000 是唯一活动任务。F-001 虽是推荐的下一用户价值任务，但必须在 B-000 关闭后由用户选择并批准任务卡。

## B-000：项目与工程基线

- 状态：`ACTIVE`
- 当前任务卡：[current-task.md](./current-task.md)
- 目标：建立文档、前后端健康骨架、依赖锁定、离线测试、统一门禁和 CI 配置；
- 验收闭环：安装依赖、启动 FastAPI 和 React、查看健康状态、运行统一门禁、从文档恢复上下文；
- 明确不做：旅行规划业务、真实 provider 调用、SQLite 业务 Schema、正式产品 UI；
- 完成后：归档任务卡并等待用户从本 roadmap 选择下一任务。
...
F-001 的真实数据验收开始前，用户需要完成或明确授权：

- 创建高德开放平台账户、应用和适用的 Key；
- 创建和风天气账户、项目和 Key，并从控制台确认 API Host；
...
该门禁目前未满足，不阻塞 B-000，也不授权 Codex 创建账户、登录、付费或调用真实 API。若用户选择 F-001 时门禁仍未满足，任务可以先完成离线端口和 UI 切片，但不能宣称真实外部服务闭环完成。
