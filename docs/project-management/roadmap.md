...
目标：在已完成的 B-000 工程底座上持续交付可恢复、可验证的本地旅行规划能力。F-001 已完成交付并归档；F-002 已由用户选择并批准，Step 0–5 已完成，Step 6 的 retry 阻塞已修复，本地全量门禁和交付审查通过；PR #6 已创建且实施提交 CI 通过，等待最终 head CI 与合并授权。
...
| 2 | F-002 计划持久化、来源与版本 | ACTIVE | 用户能保存、恢复和追踪计划版本及来源 | F-001 |
...
- 状态：`ACTIVE`（任务卡已批准；PR #6 OPEN，实施提交 CI 通过，等待最终 head CI 与合并授权）
...
- 当前执行边界：Step 1 已冻结设计；Step 2 已实现 SQLite 连接、migration runner 和初始 schema；Step 3 已实现持久化 Repository 映射；Step 4 已装配现有 POST/GET/retry API；Step 5 已实现单计划 DELETE、job 级级联、migration 后有界 30 天清理、typed acceptance record 和隐私拒绝。Step 6 已保持 attempt 1 标识兼容，并以 attempt/trace 命名空间隔离 retry 生成的 plan/source 标识；真实执行器 → SQLite → retry → 第二计划版本纵向回归和全量门禁通过。历史列表、版本比较/恢复、清空全部数据和前端历史能力仍不在范围内。
