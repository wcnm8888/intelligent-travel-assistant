...
目标：B-000、F-001、F-002、F-003、F-004A 和 F-004B1 已完成；F-005 已获批准并完成 Step 0–7，当前执行 Step 8 交付。后续顺序保持 F-005 → F-004B2，F-006 继续等待用户选择。
...
B-000 至 F-004B1 的已选任务均已完成归档。F-005 是唯一 ACTIVE 任务，Step 0–7 已完成，Step 7 的过程偏差已由用户明确接受，Step 8 正在执行；不得自动 merge、进入 Step 9 或后续候选。
...
- 状态：`ACTIVE`；Step 0–7 已完成，Step 7 过程偏差已由用户明确接受，Step 8 交付中
