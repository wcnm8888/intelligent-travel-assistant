...
目标：B-000、F-001、F-002、F-003 和 F-004A 已完成；当前只激活 F-004B1，批准顺序为 F-004B1 → F-005 → F-004B2，F-006 继续等待后续选择。
...
| 5 | F-004B1 多城市领域、用户提供的城际段与离线约束 | ACTIVE | 用户可表达 2–3 城顺序、住宿切换和用户提供的相邻城际段 | F-004A、已批准任务卡与 D-013 |
...
B-000 已完成且归档。F-004B1 已由用户选择并批准任务卡；当前每次只执行一个单独批准的 Step。
...
- 状态：`ACTIVE`；完整任务卡已批准，Step 0 已完成，Step 1 等待单独批准；
...
- 交付：四层 stacked PR，clean-restack，不 force-push；当前 Step 0 不提交、不 push、不创建 PR。
