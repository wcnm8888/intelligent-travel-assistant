...
- 状态：`DONE / ARCHIVED`
- 当前 Step：`Step 8 - 依序合并与关闭`（`DONE / PASS`）；Step 8A 为 `DONE / PASS`
- Step 0–5、Step 7、Step 8A：`DONE / PASS`；Step 6：`DONE / UAT_NOT_FORMALLY_PASSED / INCONCLUSIVE`
- 合并基线：PR #43 squash merge commit `6252193b8d3f3ed07498ff318e09b02edaca4889`，main CI run `33365971491` success
- 完整功能 main：PR #45 squash merge commit `772e82628766e5e2659ae7c705ea9c6adade9abd`，main CI run `33382187643` success
- clean-restack：#45 从 #43 合并后的干净 main 重建并完整替代 #44；#44 未合并且已关闭，无 force-push 或远程历史重写
...
- 历史入口说明：此处记录 Step 7 完成时的门禁；后续 Step 8 已获单独批准并完成，最终结果见下文。

## Step 8A 最小修复结论

- 状态：`DONE / PASS`。#43 合并后按批准规则从最新 origin/main 建立 #45 clean-restack；在 Step 8A 完成时原 #44 与 #45 均保持 OPEN；
...
- 后续入口说明：Step 8A 完成后曾停止等待用户重新批准；用户随后批准恢复 Step 8，合并与归档结果见下节。

## Step 8 最终合并与归档结论

- 用户已单独批准恢复 F-007 Step 8；恢复前重新核验 Step 0–5、Step 7 与 Step 8A 为 `DONE / PASS`，Step 6 为 `UAT_NOT_FORMALLY_PASSED / INCONCLUSIVE`，F-007 为唯一 ACTIVE，工作区干净；
- #45 当前 head `47607b2090920e78ebb004bba6ac1aea1a3836ea` 的 Windows offline CI run `33376777202` success，PR 状态为 mergeable/clean；
- #45 作为 #44 的 clean-restack 替代 PR squash merge，merge commit `772e82628766e5e2659ae7c705ea9c6adade9abd`；merge 后 main tree 与 #45 最终 head tree 完全一致；
- 完整功能 main CI run `33382187643` / job `99456833291` success；随后在 #45 与 #44 留下替代、merge 和 CI 证据，#44 未合并并已关闭；
- 最终功能范围包含已批准的两层全部改动和 Step 8A 最小修复，不包含 F-008/F-009；Schema version 2、migration 1/2、依赖/lockfile、公开 API shape 与 Provider 数据边界保持不变；
- F-007 以 `DONE / ARCHIVED` 正式关闭。Step 6 仍为 `UAT_NOT_FORMALLY_PASSED / INCONCLUSIVE`，不得把 F-007 最终交付表述为 QPS UAT PASS。
...
| Step 8 | 经单独批准后依序合并、必要 clean-restack、最终 main CI、归档与关闭 | DONE |
